...
interface CameraProjectionProps {
  cameraPosition: [number, number, number];
  targetPosition: [number, number, number];
  meshMaterial: MeshBasicMaterial | null;
  showScreen: boolean;
  visibleText: boolean;
}
...
function CameraProjection({
  ...
  showScreen,
  visibleText
}: CameraProjectionProps) {
  const secondaryCameraRef = useRef<Ppcamera | null>(null);
  ...
  useFrame((_, delta) => {
    ...
    setTimeout(() => {
      if (showScreen) {
        if (textRef.current) {
          const scaleSpeed = .75; // Speed of scaling
  ...
  return (
    <>
      ...
      <PerspectiveCamera
        ...
        fov={55}
        onUpdate={(self) => {self.lookAt(new Vector3(...targetPosition))}}
        makeDefault={false} // Don't make it the default camera
      />
      <Text color={"white"} anchorX="center" anchorY="middle"  scale={.5} position={[cameraPosition[0]-5.5,cameraPosition[1]+3.1,cameraPosition[2]+11.5]} rotation={[0,Math.PI,0]} >
      CH07
      </Text>
      {visibleText && 
      <Text
        ref={textRef}
              position={[cameraPosition[0],cameraPosition[1]+.2,cameraPosition[2]+5]}
